...
#SBATCH --qos=standby
#SBATCH --requeue
#SBATCH --array=0-26   # Array range based on the number of configurations
...
config=$(sed -n "${SLURM_ARRAY_TASK_ID}p" $config_file)
...
torchrun --nproc_per_node=6 parser.py --batch_size=22 --epochs=32 --learning_rate="$learning_rate" --activation="$activation" --pooling="$pooling"
