...
#SBATCH --qos=standby
#SBATCH --requeue
#SBATCH --array=0   # Array range based on the number of configurations
...
IFS=' ' read -r index epochs learning_rate pooling activation <<< "$config"

# Run the script with the specified parameters and redirect output and errors
torchrun --nproc_per_node=6 parser.py --batch_size=20 --epochs="$epochs" --learning_rate="$learning_rate" --pooling="$pooling" --activation="$activation"
